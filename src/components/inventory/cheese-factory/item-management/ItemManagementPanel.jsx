    ...
    addItem(itemWithTotal, {
      onSuccess: (data) => {
        ...
        sendInventoryUpdate({
          ...
          console.log('Inventory update sent to CEO Dashboard');
        }).catch(err => {
          console.error('Error sending to CEO Dashboard:', err);
        });
  ...
  const handleStatusChange = (id, newStatus) => {
    updateItemStatus({ id, status: newStatus }, {
      onSuccess: (updatedItem) => {
        ...
        // Send status update to CEO Dashboard
        if (updatedItem) {
          sendInventoryUpdate({
            company: 'Grand Berna Dairies',
            product: updatedItem.item_name,
            quantity: Number(updatedItem.quantity),
            action: 'status-update',
            value: updatedItem.total_cost
          }).catch(err => {
            console.error('Error sending status update to CEO Dashboard:', err);
          });
        }
      },
      onError: (error) => {
