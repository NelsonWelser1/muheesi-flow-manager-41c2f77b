...
const CoffeeContractTemplate = ({ editMode = false, data = {}, onDataChange = () => {} }) => {
  // Initialize state for contract details
  ...
  // State for coffee products with calculation capability
  const [coffeeProducts, setCoffeeProducts] = useState(
    data.coffeeProducts || [
      { 
        description: "Robusta Coffee Screen 18",
        quantity: 0,
        pricePerKg: 0,
        totalValue: 0
      }
  ...
  // Update parent component's data when our state changes
  useEffect(() => {
    if (editMode) {
      onDataChange('sellerDetails', sellerDetails);
      onDataChange('buyerDetails', buyerDetails);
      onDataChange('coffeeProducts', coffeeProducts);
    }
  }, [sellerDetails, buyerDetails, coffeeProducts, editMode, onDataChange]);

  // Handle coffee product changes and calculate total value
  const handleProductChange = (index, field, value) => {
    const updatedProducts = [...coffeeProducts];
    ...
    setCoffeeProducts(updatedProducts);
  };

  // Add a new coffee product row
      ...
      if (isMultiline) {
        return (
          <Textarea
            ...
            className="w-full min-h-[80px] border border-yellow-300 p-2"
          />
        );
      }
      return (
        <Input
          value={value}
          onChange={(e) => onDataChange(field, e.target.value)}
          className="border border-yellow-300 p-1"
        />
      );
    }
  ...
  const calculateGrandTotal = () => {
    return coffeeProducts.reduce((sum, product) => sum + (parseFloat(product.totalValue) || 0), 0);
  };
    ...
    <div className="max-w-4xl mx-auto bg-white p-8 border border-gray-200 shadow-sm print:shadow-none print:border-none">
      ...
      <div className="flex justify-between items-start mb-8 border-b pb-6">
        ...
        <div className="text-right">
          <h1 className="text-2xl font-bold text-yellow-700">COFFEE EXPORT CONTRACT</h1>
          <p className="text-sm text-gray-600 mt-2">
            Contract #: {editMode ? (
              <Input 
                value={data.contractNumber || "KCL-CE-2024-[XXXX]"}
                onChange={(e) => onDataChange('contractNumber', e.target.value)}
                className="border border-yellow-300 p-1 mt-1 w-full"
                placeholder="Enter contract number"
              />
            ) : (
              <span>{data.contractNumber || "KCL-CE-2024-[XXXX]"}</span>
            )}
          </p>
      ...
      {/* Parties Section */}
      <div className="mb-6">
        <h3 className="text-lg font-bold mb-2 text-yellow-700">PARTIES</h3>
            ...
            {editMode ? (
              <div className="space-y-2 mt-1">
                <Input 
                  ...
                  className="w-full border border-yellow-300 p-1"
                />
                <Input 
      ...
      <div className="mb-6">
        <h3 className="text-lg font-bold mb-2 text-yellow-700">COFFEE DETAILS</h3>
                  ...
                  {editMode && (
                    <td className="border border-gray-300 p-2 text-center">
                      <Button
                        ...
                        disabled={coffeeProducts.length === 1}
                      >
              ...
              {editMode && (
                <tr>
                  <td colSpan={5} className="p-2">
                    ...
                    >
                      <Plus className="h-4 w-4 mr-1" /> Add Coffee Product
                    </Button>
                  </td>
                </tr>
              )}
