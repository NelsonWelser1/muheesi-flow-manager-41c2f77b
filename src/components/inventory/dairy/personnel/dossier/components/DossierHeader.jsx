...
const DossierHeader = ({ dossiers }) => {
  const handlePrint = () => {
    const printContent = document.getElementById('employee-dossiers-table');
    if (!printContent) {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div class="header">
            <h2>Employee Dossiers Report</h2>
            <p>Total Records: ${dossiers?.length || 0}</p>
          </div>
  ...
  return (
    <CardHeader className="flex flex-row items-center justify-between">
      <CardTitle>Employee Dossiers</CardTitle>
