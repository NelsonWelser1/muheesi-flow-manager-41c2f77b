...
import { showErrorToast, showSuccessToast } from '@/components/ui/notifications';
...
const CoffeeInventoryRecords = ({ onBack, isKazo }) => {
  ...
  const formattedRecords = React.useMemo(() => {
    return allCoffeeInventory?.map(item => ({
      ...
      amount: item.buyingPrice * item.quantity,
  ...
  const handleExportCSV = () => {
    try {
      ...
      showSuccessToast(toast, "Records exported to CSV successfully");
    } catch (error) {
      console.error("CSV Export Error:", error);
      showErrorToast(toast, "Failed to export records to CSV");
    }
  };

  const handleExportExcel = () => {
    showErrorToast(toast, "Excel export not implemented yet");
  };

  const handleExportPDF = () => {
    showErrorToast(toast, "PDF export not implemented yet");
  };

  const handleRefresh = async () => {
    try {
      await refetch();
      showSuccessToast(toast, "Records refreshed successfully");
    } catch (error) {
      showErrorToast(toast, "Failed to refresh records");
    }
  };
            ...
            <TableBody>
              {paginatedRecords.map((record) => (
                <TableRow key={record.id}>
                  ...
                  <TableCell>{new Intl.NumberFormat('en-UG', { style: 'currency', currency: record.currency }).format(record.buyingPrice)}</TableCell>
