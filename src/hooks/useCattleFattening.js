...
export const useCattleFattening = (farmId = 'bukomero') => {
  ...
  const calculateAnalytics = (data) => {
    ...
    activePrograms.forEach(program => {
      if (program.daily_gain && program.daily_gain > 0) {
        totalDailyGain += program.daily_gain;
        validDailyGainCount++;
      } else if (program.entry_date && program.entry_weight && program.current_weight) {
        const days = differenceInDays(new Date(), new Date(program.entry_date));
        if (days > 0) {
          const gain = (program.current_weight - program.entry_weight) / days;
          if (gain > 0) {
            totalDailyGain += gain;
            validDailyGainCount++;
          }
    ...
    // Calculate average days in program
    const totalDays = activePrograms.reduce((sum, program) => {
      if (program.entry_date) {
        return sum + differenceInDays(new Date(), new Date(program.entry_date));
      }
      return sum;
    }, 0);
    
    const averageDaysInProgram = activePrograms.length > 0 ? totalDays / activePrograms.length : 0;
    ...
    activePrograms.forEach(program => {
      ...
      if (program.current_weight && program.entry_weight) {
        weightGainByBreed[program.breed].totalGain += (program.current_weight - program.entry_weight);
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('cattle-fattening-changes')
      .on('postgres_changes', { 
        event: '*', 
        schema: 'public', 
